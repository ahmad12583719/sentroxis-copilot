...
EOF
  ...
  indices:\
    - index: "wazuh-archives-4.x-%{+yyyy.MM.dd}"\
      when.contains:\
        tags: "wazuh-archives"\
    - index: "wazuh-alerts-4.x-%{+yyyy.MM.dd}"' "$file"
  fi
  # Upgrade configurations generated by earlier Sentroxis releases without
  # duplicating the marked routing block on an installer rerun.
  sed -i 's#wazuh-archives-%{+yyyy.MM.dd}#wazuh-archives-4.x-%{+yyyy.MM.dd}#g; s#wazuh-alerts-%{+yyyy.MM.dd}#wazuh-alerts-4.x-%{+yyyy.MM.dd}#g' "$file"
  grep -q '/var/ossec/logs/archives/archives.json' "$file" || fatal "Archive input was not added to $file"
  grep -q 'wazuh-archives-4.x-%{+yyyy.MM.dd}' "$file" || fatal "Archive routing was not added to $file"
  grep -q 'wazuh-alerts-4.x-%{+yyyy.MM.dd}' "$file" || fatal "Alert routing was not added to $file"
}
...
COPY config/wazuh-template.json /etc/filebeat/wazuh-template.json
RUN chmod go-w /etc/filebeat/filebeat.yml /etc/filebeat/wazuh-template.json
DOCKERFILE
...
DOCKERFILE
  ...
  if [[ -f "${WAZUH_HOME}/build-docker-images/wazuh-manager/config/filebeat.yml" ]]; then
  ...
  if [[ -f "$wazuh_alerts_template" ]]; then
    mkdir -p "$manager_compose_dir/config"
    cp -p "$wazuh_alerts_template" "$manager_compose_dir/config/wazuh-template.json"
  fi
}
