...
import { Check, Download, FileCode2, Loader2, RefreshCw, ShieldCheck, TerminalSquare } from 'lucide-react'
...
export default function VelociraptorInstaller({ endpoint = '' }) {
  ...
  const generateConfig = async (event) => {
    ...
    if (data) {
      setServer(data)
      setNotice(`Server, endpoint client, and API client configurations created. Initial Velociraptor administrator: ${data.admin_username}`)
    }
  }

  return <section className="panel installer-panel">
    <div className="installer-heading"><div><p className="eyebrow"><TerminalSquare size={13} /> Velociraptor runtime</p><h2>Install, configure, then run</h2><p>Self-signed TLS is the only permitted deployment mode in this workflow. The client frontend is fixed to port <strong>8010</strong>; all listed operational choices remain under the operator’s control.</p></div><span className="release-badge">release v{catalog.release}</span></div>
    <div className="installer-flow">
      <div className={`installer-card ${installation ? 'done' : ''}`}><div className="installer-card-top"><span className="installer-index">01</span>{installation ? <StepBadge state="done">Verified</StepBadge> : <StepBadge state="pending">Waiting</StepBadge>}</div><Download size={18} className="installer-card-icon" /><h3>Install official binary</h3><p>Download an allowlisted release for the selected platform and verify its SHA-256 before use.</p><label htmlFor="vr-platform">Target platform</label><select id="vr-platform" value={platform} onChange={(event) => setPlatform(event.target.value)} disabled={busy !== ''}>{catalog.assets.map((item) => <option key={item.platform} value={item.platform}>{item.platform}{item.is_host_platform ? ' · host' : ''}</option>)}</select><button className="button secondary installer-action" onClick={prepare} disabled={busy !== ''}>{busy === 'download' ? <Loader2 size={14} className="spin" /> : <Download size={14} />}{installation ? 'Verify again' : 'Download & verify'}</button>{asset && <small className="hash-line">SHA-256 {asset.sha256.slice(0, 16)}…</small>}</div>
      <div className="installer-card"><div className="installer-card-top"><span className="installer-index">02</span>{server?.client_config_path ? <StepBadge state="done">Generated</StepBadge> : <StepBadge state="pending">Required</StepBadge>}</div><FileCode2 size={18} className="installer-card-icon violet" /><h3>Generate server, client &amp; API config</h3><p>Only the deployment type, frontend port, DNS mode, and initial administrator identity are automated. The remaining values below are selected by the operator.</p><form onSubmit={generateConfig} className="terminal-input installer-config-form"><label htmlFor="vr-server-os">Server operating system</label><select id="vr-server-os" value={settings.server_os} onChange={(event) => updateSetting('server_os', event.target.value)} disabled={!installation || busy !== ''}><option value="linux">Linux</option><option value="windows">Windows</option><option value="darwin">macOS</option></select><label htmlFor="vr-datastore">Datastore directory</label><input id="vr-datastore" value={settings.datastore_path} onChange={(event) => updateSetting('datastore_path', event.target.value)} placeholder={settings.server_os === 'windows' ? 'C:\\Velociraptor' : '~/.sentroxis/velociraptor'} required disabled={!installation || busy !== ''} /><label htmlFor="vr-logs">Logs directory <small>optional</small></label><input id="vr-logs" value={settings.log_path} onChange={(event) => updateSetting('log_path', event.target.value)} placeholder="Defaults to <datastore>/logs" disabled={!installation || busy !== ''} /><label htmlFor="vr-cert-years">Internal certificate lifetime</label><select id="vr-cert-years" value={settings.certificate_years} onChange={(event) => updateSetting('certificate_years', event.target.value)} disabled={!installation || busy !== ''}><option value="1">1 year</option><option value="2">2 years</option><option value="10">10 years</option></select><label htmlFor="vr-hostname">Public frontend DNS name or server IP</label><input id="vr-hostname" value={settings.frontend_hostname || endpointHostname} onChange={(event) => updateSetting('frontend_hostname', event.target.value)} placeholder="192.168.1.10 or velo.example.com" required disabled={!installation || busy !== ''} /><label htmlFor="vr-gui-port">GUI port</label><input id="vr-gui-port" type="number" min="1" max="65535" value={settings.gui_port} onChange={(event) => updateSetting('gui_port', event.target.value)} required disabled={!installation || busy !== ''} /><label className="installer-checkbox"><input type="checkbox" checked={settings.use_websocket} onChange={(event) => updateSetting('use_websocket', event.target.checked)} disabled={!installation || busy !== ''} /> Use experimental WebSocket communications</label><label className="installer-checkbox"><input type="checkbox" checked={settings.use_registry_writeback} onChange={(event) => updateSetting('use_registry_writeback', event.target.checked)} disabled={!installation || busy !== ''} /> Use the Windows registry for client writeback</label><label htmlFor="vr-frontend-port">Frontend port <small>fixed by Sentroxis</small></label><input id="vr-frontend-port" value="8010" readOnly aria-readonly="true" /><label htmlFor="vr-admin">Initial Velociraptor administrator</label><input id="vr-admin" value={user?.email || ''} readOnly aria-readonly="true" /><label htmlFor="vr-password">Confirm current Sentroxis password</label><input id="vr-password" type="password" autoComplete="current-password" value={settings.password_confirmation} onChange={(event) => updateSetting('password_confirmation', event.target.value)} placeholder="Used once; never stored" required disabled={!installation || busy !== ''} /><button className="button secondary installer-action" type="submit" disabled={!installation || busy !== ''}>{busy === 'generate' ? <Loader2 size={14} className="spin" /> : <FileCode2 size={14} />}Generate all configuration files</button></form></div>
      <div className="installer-card done"><div className="installer-card-top"><span className="installer-index">03</span><StepBadge state="done">Managed</StepBadge></div><ShieldCheck size={18} className="installer-card-icon green" /><h3>Server lifecycle</h3><p>The project-local server runs through <code>./startup.sh</code>. When Sentroxis stops with <code>Ctrl+C</code>, its Velociraptor process stops too.</p><code className="command-chip">./velociraptor --config server.config.yaml frontend -v</code><p className="hash-line">Open the Velociraptor dashboard for live server status and the local GUI.</p></div>
    </div>
    {notice && <div className="installer-footer"><span><Check size={13} /> {notice}</span></div>}
    {server?.api_config_path && <div className="installer-footer"><a className="text-button" href="/api/velociraptor/api-config/download"><Download size={13} /> Download API configuration</a><span>Saved as `api.config.yaml`</span></div>}
    {error && <div className="installer-error" role="alert"><RefreshCw size={14} /> {error}</div>}
    <div className="installer-footer"><span><Check size={13} /> Self-signed TLS and manual DNS only</span><span><ShieldCheck size={13} /> Frontend client port: 8010</span><span>GUI port remains operator-selected</span></div>
  </section>
}
